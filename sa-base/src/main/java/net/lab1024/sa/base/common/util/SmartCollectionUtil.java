...
public class SmartCollectionUtil {
    ...
    public static <T> Integer getIntegerSumValue(Collection<T> from, Function<T, Integer> valueFunc) {
    ...
    public static <T> Integer getIntegerSumValue(Collection<T> collection,
                                                 Function<T, Integer> mapper,
                                                 Integer defaultValue) {
        if (CollUtil.isEmpty(collection)) {
            return defaultValue;
        }
        assert !collection.isEmpty(); // 断言，避免告警
        // 核心：null值按0处理，有效数值保留正负号（支持加减）
        return collection.stream()
                .map(mapper)
                .map(val -> Objects.isNull(val) ? 0 : val) // null→0，不影响加减
                .reduce(defaultValue, Integer::sum);
    }
//
//    public <T> Integer getIntegerSumValue(Collection<T> collection, Function<T, Integer> mapper, Integer defaultValue) {
//        if (CollUtil.isEmpty(collection)) {
//            return defaultValue;
//        }
//        assert !collection.isEmpty(); // 断言，避免告警
//        return collection.stream()
//                .map(mapper)
//                .map(val -> Objects.isNull(val) ? 0 : val) // 空值兜底为0
//                .reduce(defaultValue, Integer::sum);
//    }
    ...
    public static <K, T> Map<K, List<T>> groupingBy(List<T> list, Function<? super T, ? extends K> keyMapper) {
        ...
        if (CollUtil.isNotEmpty(list)) {
            retMap = list.stream().collect(Collectors.groupingBy(keyMapper));
        }
