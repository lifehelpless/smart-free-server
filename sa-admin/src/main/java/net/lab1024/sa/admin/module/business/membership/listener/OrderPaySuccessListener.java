...
public class OrderPaySuccessListener {
    ...
    public void onPaySuccess(PaySuccessEvent event) {
        ...
        // 1. 查询订单
        MemberOrderEntity order = memberOrderService.selectOne(Wrappers.<MemberOrderEntity>lambdaQuery().eq(MemberOrderEntity::getOrderNo, orderNo));
        if (order == null) {
        ...
        // 2. 幂等性校验 (防止重复消费)
        if (!MemberOrderStatusEnum.WAIT_PAY.getCode().equals(order.getStatus())) {
        ...
        // 4. 更新订单状态为已支付
        order.setStatus(MemberOrderStatusEnum.PAID.getCode());
